...
energygrps=Protein Solvent
;energygrp_excl=Protein Protein Membrane Membrane

; for gmx_mpi5, need cutoff-scheme = Group to allow energy group exclusions
...
# Run
M="gmx_mpi mdrun -deffnm $BASE -v -nt $NP"
echo $M; $M 
...
do
  ...
  echo $G; $G || exit
  M="gmx_mpi mdrun -deffnm $tag -v -nt $NP"
  echo $M; $M 
  GRO=$tag.gro
