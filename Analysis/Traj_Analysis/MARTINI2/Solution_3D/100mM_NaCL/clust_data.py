...
"""
print("Reading data...")
data_fes = [line.split() for line in open(sys.argv[6], "r")]
...
for i in range(0,n_values):
    ...
    # if data['Min dist'][i] < cut_off and data['RMSD-B'][i] < border:
        ...
        langle_dist_mat.append([data['dist'][i],math.degrees(data['Ax-ang'][i])])
        memb_dist_mat.append(data['Memb dist'][i])
        min_dist.append(data['Min dist'][i])
        timesteps.append(data['Timestep'][i])
        dim_ang_mat.append(data['Dim_ang-1'][i])
        memb_ang_mat.append(data['Rot_angle'][i])
        count+=1
...
for i in range(num_clusters):
    ...
    for j in range(len(fr_num)):
        ...
        dim_angle.append(dim_ang_mat[fr_num[j]])
        axial_ang.append(langle_dist_mat[fr_num[j],1])
    ...
    # ax_01[0].plot(free_val,label_colours[i],linewidth=0.6)
...
for i in range(num_clusters):
    ...
    for j in range(len(fr_num)):
        ...
        free_val.append(free_mat[fr_num[j],2])
        d3.append(langle_dist_mat[fr_num[j],0])
        memb_angle.append(memb_ang_mat[fr_num[j]])


    #ax_01[0].plot(d3,'r',linewidth=0.6)
    lb_str = "C-" + str(i)
    ax_dis[0].hist(d3,bins=100,density=True,histtype='stepfilled',color=label_colours[i],alpha=0.6,label=lb_str)
    ax_dis[1].hist(rmsd_val,bins=100,density=True,histtype='stepfilled',color=label_colours[i],alpha=0.6,label=lb_str)
    ...
    ax_dis[3].hist(memb_angle,bins=50,histtype='stepfilled',color=label_colours[i],alpha=0.6,label=lb_str)
...
ax_dis[1].axvline(x=0.9,linestyle='--',linewidth=0.8)
# ax_dis[3].axvline(x=148.65,linestyle='--',linewidth=0.8)
ax_dis[2].axvline(x=3.33,linestyle='--',linewidth=0.8)
ax_dis[3].axvline(x=22.0,linestyle='--',linewidth=0.8)
ax_dis[0].set_ylabel('Frequency')
ax_dis[1].set_ylabel('Frequency')
ax_dis[2].set_ylabel('Frequency')
ax_dis[3].set_ylabel('Frequency')
ax_dis[0].set_title('dc')
ax_dis[1].set_title('RMSD')
ax_dis[3].set_title('Orientation')
