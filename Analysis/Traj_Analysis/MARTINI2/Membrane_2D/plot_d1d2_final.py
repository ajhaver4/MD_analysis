...
com_data = pandas.read_csv(sys.argv[2],comment='#',sep='\t')
timesteps_array = data['Timestep']/1e6


mask_bound = data['RMSD-B'] < 2.0
...
com_filter_data = com_data[::interval]
...
Plot d1
"""
color_map = np.where(mask1 , 'firebrick', 'silver')
...
fig,[ax,ax1]=plt.subplots(2,1,sharex=True,figsize=fig_size)
...
# ax.plot(data['Timestep'][::interval]/1e6,data['d1'][::interval],'crimson',linewidth=2.0,label=r'$d_1$',linestyle='-')
ax.plot(com_filter_data['Timestep']/1e6,com_filter_data['comA'],linewidth=lw,linestyle='-',color='sienna',alpha=0.8)
