...
pro_A_cry = cry_univ.select_atoms("bynum 1-469")
pro_B_cry = cry_univ.select_atoms("bynum 470-938")
...
for chunk in chunks:
    ...
    aligner_A = align.AlignTraj(u, cry_univ,select='bynum 1-469 and name BB',in_memory=True).run()
    ...
    for i in range(chunk+step):
        ...
        residualA = distances.dist(pro_A_bb,proA_bb_cry,box=box_dims)
        print(residualA.shape)
...
rms_residuals = np.sqrt(avg_residuals)/10.0             #Convert to nm
