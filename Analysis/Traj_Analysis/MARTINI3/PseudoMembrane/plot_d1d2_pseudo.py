...
Plot d1
"""
color_map = np.where(mask1 , 'teal', 'silver')
...
# ax.plot(data['Timestep'][::interval]/1e6,data['d1'][::interval],'crimson',linewidth=2.0,label=r'$d_1$',linestyle='-')
ax.plot(com_filter_data['Timestep']/1e6,com_filter_data['COMz_chainA'],linewidth=lw,linestyle='-',color='sienna',alpha=0.8)
...
ax.tick_params(labelsize=t_size)
fig.tight_layout()
figlbl = "COM_FINAL_" + str(disp_ts) + "ns.svg"
plt.savefig(figlbl,dpi=600,format="svg",transparent=True)
plt.show()
