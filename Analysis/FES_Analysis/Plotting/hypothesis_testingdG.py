...
std_timestamps3D_ctrl = np.arange(10,50,3)
...
hist_valuesPs = [val for dG_values in dG_blocksPs.values() for val in dG_values]
...
hyp_estimate2D = [val - 17.97 for val in hist_values3D] 
hyp_estimatePs = [val - 23.33 for val in hist_values3D]
print(len(hist_values3D), len(std_timestamps))
print(len(hist_values2D), len(std_timestamps2D))
print(len(hist_valuesPs), len(std_timestamps))
...
def get_pvalues(estimated_values,actual_values,ax=None,label=None,color=None):
    ...
    ax.hist(estimated_values,bins=50,label=r'$\Delta G_{rigid}$',alpha=0.5,color='gray',density=True)
...
fig.tight_layout()
fig.savefig('hvalues_test_2D.png',dpi=600,transparent=True,format='png')
fig2,ax2 = plt.subplots(figsize=(6,4))
ks_statPs,p_valuePs = get_pvalues(hyp_estimatePs,hist_valuesPs,ax=ax2,label=r'$\Delta G_{2D-Ps}$',color='teal')
fig2.tight_layout()
fig2.savefig('hvalues_test_Ps.png',dpi=600,transparent=True,format='png')
...
with open('dG_Rigid_2D', 'w') as fl:
    fl.write('#dG_rigid\n')
    for val in hyp_estimate2D:
        fl.write(f'{val}\n')

with open('dG_FES_2D', 'w') as fl:
    fl.write('#dG_2D\n')
    for val in hist_values2D:
        fl.write(f'{val}\n')

with open('dG_Rigid_Ps', 'w') as fl:
    fl.write('#dG_rigid\n')
    for val in hyp_estimatePs:
        fl.write(f'{val}\n')

with open('dG_FES_2DPs', 'w') as fl:
    fl.write('#dG_Ps\n')
    for val in hist_valuesPs:
        fl.write(f'{val}\n')
...
print("FOr 3D and 2D-Ps distributions: ")
print(f"KS Statistic: {ks_statPs}")
print(f"P-value: {p_valuePs}")
if p_valuePs > alpha:
    print("The null hypothesis cannot be rejected. The distributions are likely from the same distribution.")
else:
    print("The null hypothesis can be rejected. The measurements are likely from different distributions which indicates no significant overlap")
...
"""
#Shift 3D values by the required ddG so that the resulatant distribution can be compared to the actual 2D distribution for hypothesis testing
hyp_estimate2D = [val - 17.97 for val in avg_values3D.values()] 
hyp_estimatePs = [val - 23.33 for val in avg_values3D.values()]
...
ks_statPs,p_valuePs = get_pvalues(hyp_estimatePs,list(avg_valuesPs.values()),ax=ax2,label=r'$\Delta G_{2D-Ps}$',color='teal')
