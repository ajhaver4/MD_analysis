...
data_d1 = pd.read_csv(sys.argv[1],delimiter="\s+",names=['d1','fes','bias'],comment='#')
data_d2 = pd.read_csv(sys.argv[2],delimiter="\s+",names=['d1','fes','bias'],comment='#')
data_d3 = pd.read_csv(sys.argv[3],delimiter="\s+",names=['d1','fes','bias'],comment='#')
data_d4 = pd.read_csv(sys.argv[4],delimiter="\s+",names=['d1','fes','bias'],comment='#')
data_d5 = pd.read_csv(sys.argv[5],delimiter="\s+",names=['d1','fes','bias'],comment='#')
data_d6 = pd.read_csv(sys.argv[6],delimiter="\s+",names=['d1','fes','bias'],comment='#')
...
def plot_1Dfes(data,label,ax,alpha=1.0,color='royalblue',lw=1,ls='-'):
    ...
    mask_zero = (data['d1'] > 16.0) & (data['d1'] < 18.0)
...
def plot_1Dfes_PS(data,label,ax,alpha=1.0,color='royalblue',lw=1,ls='-'):

        mask_zero = (data['d1'] > 12.0)
        zero_val = np.mean(data['fes'][mask_zero])
        data['fes'] = data['fes'] - zero_val
        ax.plot(data['d1'],data['fes'],label=label,alpha=alpha,color=color,linewidth=lw,linestyle=ls)
...
# input_labels=['3D_d1','3D_d2','2D_d1','2D_d2']
...
plt.savefig("FES_d1_FINAL_PS.svg",dpi=600,format="svg",transparent=True)
plt.savefig("FES_d1_FINAL_PS.png",dpi=600,format="png",transparent=True)
...
fig.tight_layout()
plt.savefig("FES_d2_FINAL_PS.svg",dpi=600,format="svg",transparent=True)
plt.savefig("FES_d2_FINAL_PS.png",dpi=600,format="png",transparent=True)
...
ax1.set_xlim(left=0.4,right=19.0)
ax1.set_ylim(bottom=np.min(data_d3['fes'])-10,top=25)
ax1.tick_params(labelsize=t_size)
ax1.legend(fontsize=leg_size,loc ='lower right')


ax1.vlines(0.81,np.min(data_d3['fes'])-10,0,linestyle='--',color='black',linewidth=2.0)
...
ax2.set_xlim(left=0.4,right=19.0)
ax2.set_ylim(bottom=np.min(data_d3['fes'])-10,top=25)
ax2.tick_params(labelsize=t_size)
ax2.legend(fontsize=leg_size,loc ='lower right')


ax2.vlines(0.81,np.min(data_d3['fes']),0,linestyle='--',color='black',linewidth=2.0)

#2D-Ps
plot_1Dfes_PS(data_d5,r'$d_1$',ax3,alpha=0.8,color='teal',lw=lw)
plot_1Dfes_PS(data_d6,r'$d_2$',ax3,alpha=0.8,color='teal',lw=lw,ls='dotted')

ax3.set_xlabel(r'$\vec{s} (nm)$',fontsize=f_size)    
ax3.set_ylabel(r"$F_{2DPs}(\vec{s})$",fontsize=f_size)
ax3.set_xlim(left=0.4,right=19.0)
ax3.set_ylim(bottom=np.min(data_d5['fes'])-10,top=25)

ax3.tick_params(labelsize=t_size)
ax3.legend(fontsize=leg_size,loc ='lower right')


ax3.vlines(0.81,np.min(data_d5['fes']),0,linestyle='--',color='black',linewidth=2.0)


fig.tight_layout()
# plt.show()
plt.savefig("FES_Combined_PS.svg",dpi=600,format="svg",transparent=True)
plt.savefig("FES_Combined_PS.png",dpi=600,format="png",transparent=True)
...
dG_2D_d1_ps = get_min_fes(data_d5)
dG_2D_d2_ps = get_min_fes(data_d6)

print("dG 3D d1: ",dG_3D_d1)
print("dG 3D d2: ",dG_3D_d2)
print("dG 2D d1: ",dG_2D_d1)
print("dG 2D d2: ",dG_2D_d2)
print("dG 2D d1 PS: ",dG_2D_d1_ps)
print("dG 2D d2 PS: ",dG_2D_d2_ps)