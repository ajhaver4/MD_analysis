...
def integrate_fes(bias_pot,states,dG,dG_unb_calc,d1_bins,d2_bins):
    k = constants.value('Boltzmann constant')
    Ava_no = constants.value('Avogadro constant')
    temp = 310
    kbt = (k*temp*Ava_no)/1000   #kJ/mol
    ...
    for st,coords in states.items():
        ...
        # Z = np.sum(np.exp(-1*bias2/kbt)*dx*dy)
        Z_states[st]=Z
...
def integrate_1D(bias_pot,d1_bins,d2_bins):
    k = constants.value('Boltzmann constant')
    Ava_no = constants.value('Avogadro constant')
    temp = 310
    kbt = (k*temp*Ava_no)/1000   #kJ/mol
    ...
    for i in range(len(d1_bins)):
        d1_fes[i] = -kbt*np.log(np.sum(np.exp(-1*bias_pot[i,:]/kbt)*dy))
        d1_bias[i] = np.sum(bias_pot[i,:])

    for i in range(len(d2_bins)):
        d2_fes[i] = -kbt*np.log(np.sum(np.exp(-1*bias_pot[:,i]/kbt)*dx))
        d2_bias[i] = np.sum(bias_pot[:,i])
...
for idx in range(len(data['time'])):
    ...
    # dG,dG_unb_calc = integrate_fes(bias_pot,states,dG,dG_unb_calc,d1_bins,d2_bins)
    dG,dG_unb_calc = integrate_fesUnb(bias_pot,states,dG,dG_unb_calc,d1_bins,d2_bins)
