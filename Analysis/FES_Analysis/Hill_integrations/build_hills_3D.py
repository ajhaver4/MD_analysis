...
welltemp_time_min = float(sys.argv[4])*1e6
welltemp_time_max = float(sys.argv[5])*1e6
bias_f = float(sys.argv[6])
...
welltemp_bias = np.zeros((len(d1_bins),len(d2_bins)))

fes_grid = np.zeros((len(d1_bins),len(d2_bins)))



def integrate_fesUnb(fesgrid,states,dG,dG_unb_calc,d1_bins,d2_bins):
    ...
    for st,coords in states.items():
        ...
        mask1_unb = (d1_bins>=xmax) & (d1_bins<=21.0)
        mask2_unb = (d2_bins>=ymax) & (d2_bins<=21.0)
        #Bound State 
        F= fesgrid[mask2,:]
        F_of_s=F[:,mask1]
        ...
        F = fesgrid[mask2_unb,:]
        F_of_s=F[:,mask1_unb]
        Z_unb = np.sum(np.exp(-1*F_of_s/kbt)*dx*dy)

        dG_state = -kbt*np.log(Z/Z_unb)
        dG[st].append(dG_state)

    return(dG,dG_unb_calc)
...
def integrate_fes(fesgrid,states,dG,dG_unb_calc,d1_bins,d2_bins):
    ...
    for st,coords in states.items():
        ...
        F1 = fesgrid[mask2,:]        #Select all bins satisfying d2
        F2 = fesgrid[:,mask1]        #Select all bins satisfying d1
...
for idx in range(len(data['time'])):
    ...
    if time < welltemp_time_min or time > welltemp_time_max:
        # bias_x = ((d1_grid+bin_width_d1*0.5) - curr_d1)**2/(2*(s1**2))
        # bias_y = ((d2_grid+bin_width_d2*0.5) - curr_d2)**2/(2*(s2**2))
        # expo = np.exp(-1*(bias_x+bias_y))
        ...
        # bias_y = ((d2_grid+bin_width_d2*0.5) - curr_d2)**2/(2*(s2**2))
        # expo = np.exp(-1*(bias_x+bias_y))
        welltemp_bias+= -1*h*avg_scaling*expo  

    fes_grid = bias_pot +wt_scaling*welltemp_bias
    # dG,dG_unb_calc = integrate_fes(fes_grid,states,dG,dG_unb_calc,d1_bins,d2_bins)
    dG,dG_unb_calc = integrate_fesUnb(fes_grid,states,dG,dG_unb_calc,d1_bins,d2_bins)
